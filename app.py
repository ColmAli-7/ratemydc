...
def admin():
    ...
    if teacher_id:
        ...
        reviews_query = reviews_query.join(Teacher).filter(Teacher.subject_id == int(subject_id))
...
def admin_teacher_detail(teacher_id):
    ...
    reviews = Review.query.filter_by(teacher_id=teacher.id).order_by(Review.id.desc()).all()
